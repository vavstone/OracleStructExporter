...
namespace ServiceCheck.Core
{
    public class LoggerOuter
    {
        ...
        public LoggerOuter(TextFilesLog textFilesLog, DBLog dbLog = null)
        {
        ...
        public bool IsNecessaryToInsertLogEntry(ExportProgressDataLevel progressDataLevel,
            ExportProgressDataStageOuter progressDataStage, LogType logType)
        {
            ...
            if (!curLog.ExcludeStageOuterInfoC.Any())
                return true;
            foreach (var stage in curLog.ExcludeStageOuterInfoC.Keys)
            {
                if (stage == progressDataStage &&
                    (curLog.ExcludeStageOuterInfoC[stage] == ExportProgressDataLevel.NONE ||
                     curLog.ExcludeStageOuterInfoC[stage] == progressDataLevel))
                    return false;
            }
        ...
        public void InsertThreadsTextFileLog(ExportProgressDataOuter progressData, bool checkOnNecessaryBySettings,
            out string messageText)
        {
            ...
            {
                //готовим текст
                if (progressData.Stage == ExportProgressDataStageOuter.PROCESS_SCHEMA &&
        ...
        public void InsertRepoTextFileLog(ExportProgressDataOuter progressData, bool checkOnNecessaryBySettings)
        {
        ...
        public void InsertMainTextFileLog(ExportProgressDataOuter progressData, bool checkOnNecessaryBySettings)
        {
            ...
            {
                //готовим текст
                if (progressData.Stage == ExportProgressDataStageOuter.PROCESS_MAIN &&
                    progressData.Level == ExportProgressDataLevel.STAGESTARTINFO)
                {
                    //вставляем стартовый разделитель
                    //messageText += Environment.NewLine;
        ...
        public void InsertThreadsDBLog(ExportProgressDataOuter progressData, bool checkOnNecessaryBySettings,
        ...
        public void InsertRepoDBLog(ExportProgressDataOuter progressData, bool checkOnNecessaryBySettings,
            string connectionString, DBLog dbLogSettings)
        {
            ...
            //    IsNecessaryToInsertLogEntry(progressData.Level, progressData.Stage, LogType.DBLog))
            {
                ...
                //{
                    //сохраняем в файл запись лога
                    DbWorkerOuter.SaveRepoChangesInDB(_dbLog.DBLogPrefix, progressData, connectionString/*, dbLogSettings*/, saveDetails);
                //}
            }
        ...
        public string GetToWorkInfo(PrognozBySchemaOuter prognozItem, bool usePrognozInfo)
        {
            ...
            table.DataRows = dataRows;
                ...
                if (usePrognozInfo)
                {
                    dataRow.Add(new DataCell
                    {
                        Content = prognozItem.OjectsCount == null ? "-" : prognozItem.OjectsCount.Value.ToStringFormat(0),
                        ColumnId = "objectscount"
                    });
                    dataRow.Add(new DataCell
                    {
                        Content = prognozItem.DurationsInMinutes == null
                            ? "-"
                            : prognozItem.DurationsInMinutes.Value.ToStringFormat(1),
                        ColumnId = "duration"
                    });
                }
            ...
            if (usePrognozInfo)
            {
                var commonPrognozEnd = PrognozBySchemaOuter.CommonPrognozEnd(prognozItem, DateTime.Now);
