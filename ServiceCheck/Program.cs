...
namespace ServiceCheck.Scheduler
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                var statFullList = exporter.GetAggrFullStat(settings.SchedulerSettings.ConnectionsToProcess.ConnectionListToProcess, settings.SchedulerSettings.GetStatForLastDays, prefixForGettingStat);
                

                //var statInfoShort = logger.GetStatInfo(statList, getStatForLastDays);
                var statInfoShort = logger.GetStatInfoV2(statFullList, settings.SchedulerSettings.GetStatForLastDays);
                var statInfoFull = logger.GetStatFullInfo(statFullList, settings.SchedulerSettings.GetStatForLastDays);
                ...
                var statSplitPeriodAppend = Logger.PeriodAppendToLogFileName(settings.TextFilesLog.StatSplitPeriod);
                ...
                var connectionsToProcess = Exporter.SelectConnectionsToProcess(
                    statFullList, 
                    settings.SchedulerSettings.ConnectionsToProcess.MaxConnectPerOneProcess,
                    settings.SchedulerSettings.MinSuccessResultsForStat);
                ...
                else
                {
                    ...
                    exporter.StartWork(threads, schemasToWorkInfo, false, settings.TestMode);
                }
